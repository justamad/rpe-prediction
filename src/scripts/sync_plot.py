...
def plot_sync_data(pos_df: pd.DataFrame, imu_df: pd.DataFrame):
    ...
    fix, axs = plt.subplots(2, 1, figsize=(ps.text_width * ps.cm, ps.text_width * 0.6 * ps.cm), dpi=ps.dpi)
...
def fusing_plots(master_path: str, sub_path: str, joint_col: str):
    ...
    plt.plot(df1[joint_col], label="Left")
    plt.plot(df2[joint_col], label="Right")
    plt.plot(fused_df[joint_col], label="Fused")

    plt.xlabel("Time [s]")
    plt.ylabel("Position [m]")
    plt.legend(loc="upper right")
    plt.tight_layout()
    plt.savefig("fusing.pdf", dpi=ps.dpi)
