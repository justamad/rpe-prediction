...
class ShakeExtractor(AbstractExtractor):
    ...
    def get_peak_sequences(self, signals, column, start_window, end_window):
        ...
        logger.debug(
            '{} sequences did satisfy minimum length of {} for {}'.format(len(sequences_filtered), self.min_length,
                                                                          column))
    ...
    def get_segments(self, signals):
        ...
        time_buffer = pd.Timedelta(
            seconds=self.time_buffer)  # will be added to start and subtracted from end of sequence
