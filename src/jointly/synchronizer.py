...
class Synchronizer(object):
    ...
    def __init__(self, sources, ref_source_name, extractor, sampling_freq=None, tags=None):
        ...
        print(f"Calculated sampling freq: {self.sampling_freq}")
    ...
    @staticmethod
    def _get_stretch_factor(segments, time_shifts):
        old_length = segments['second']['start'] - segments['first']['start']
        new_length = old_length + time_shifts['second'] - time_shifts['first']
        stretch_factor = new_length / old_length
        return stretch_factor

    @staticmethod
    def _get_time_shifts(dataframe, columns, segments):
        """
        Returns time shifts to synchronize columns[1] with columns[0].
        ...
        for index, segment in enumerate(segment_names):
            logger.debug(f'Calculate time shift of {segment} segment for {sig_column} to {ref_column}.')
            ...
            logger.debug(f'Highest correlation with start at {max_corr_ts} with {np.max(cross_corr)}.')

            # calculate time shift to move signal to maximize correlation
            time_shifts[segment] = max_corr_ts - sig_start
            logger.debug('Timeshift is {}.'.format(str(time_shifts[segment])))

            # plot shifted segments
            if logger.isEnabledFor(logging.INFO):
                df = dataframe.copy()
                df[sig_column] = df[sig_column].shift(1, freq=time_shifts[segment])
    ...
    def _calculate_sync_params(self):
        ...
        for column in df_equi.columns:
            if column == self.ref_source_name:
                ...
                time_shifts = Synchronizer._get_time_shifts(df_equi, [self.ref_source_name, column], segments)
                logger.debug(
                    'Timedelta between shifts before stretching: {}'.format(time_shifts['first'] - time_shifts['second']))
                self.sources[column]['stretch_factor'] = Synchronizer._get_stretch_factor(segments[column], time_shifts)
                ...
                signal_stretched = Synchronizer._stretch_signals(pd.DataFrame(dataframe[column]),
                                                                 self.sources[column]['stretch_factor'], start_time)
        ...
        # Second round to get time shift for stretched signal
        for column in df_equi.columns:
            if column == self.ref_source_name:
                continue
            else:
                time_shifts = Synchronizer._get_time_shifts(df_equi, [self.ref_source_name, column], segments)
                timedelta = time_shifts['first'] - time_shifts['second']
                if timedelta > pd.Timedelta(0):
                    logger.warning('Timedelta between shifts after stretching: {}'.format(timedelta))
                logger.info('Timeshift for {}: {}'.format(column, time_shifts['first']))
                self.sources[column]['timeshift'] = time_shifts['first']
    ...
    def save_data(self, path, tables=None, save_total_table=True):
            ...
            for table_name, table_spec in tables.items():
                ...
                for source_name, source_columns in table_spec.items():
                    ...
                    if not source_df.empty:
                        ...
                        source_df = source_df.rename(lambda x: '{prefix}_{column}'.format(prefix=source_name, column=x),
                                                     axis='columns')
        ...
        if save_total_table:
            ...
            for source_name, data in synced_data.items():
                source_df = data.rename(lambda x: '{prefix}_{column}'.format(prefix=source_name, column=x),
                                        axis='columns')
