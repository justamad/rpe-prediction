...
def calculate_kinect_data(iterator, mode="train"):
    ...
    for set_data in iterator:
        ...
        data = filter_dataframe(data, ["pos"])
        data = normalize_mean(data, std_dev_factor=1.0)
        # plot_sensor_data_for_axes(data, "Position_data", f"{set_data['azure']}_position.png")
        # segment_exercises_based_on_joint(positions[], )
        # orientations = filter_dataframe(azure.orientation_data, excluded_joints)
        features = calculate_features_sliding_window(data, window_size=30, step_size=3)
