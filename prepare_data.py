...
def prepare_skeleton_data(iterator):
    ...
    for set_data in iterator:
        ...
        x_data = filter_dataframe(azure._data, excluded_joints)
        # data = filter_dataframe(data, ["timestamp", "pos"])
        x_data = normalize_mean(x_data, std_dev_factor=1.0)
        plot_sensor_data_for_axes(x_data, "Position_data", azure.get_skeleton_joints(), f"{set_data['azure']}_pos.png")
        # segment_exercises_based_on_joint(positions[], )
        # orientations = filter_dataframe(azure.orientation_data, excluded_joints)
        features = calculate_features_sliding_window(x_data, window_size=30, step_size=2)
